...
public class ListeDePokemon implements Serializable {
    ...
    public Pokemon getPokemonByEnvironnementAndPourcentage (Environnement environnement, int pourcentage) {
        ...
        /*for (int i = 0 ; i < this.pokemons.size() ; i++) {
            System.out.println("pokemon : " + this.pokemons.get(i).getNom() + " ; pourcentage : " + this.pourcentage.get(i));
        }*/
        if (pourcentage >= pourcentageApparition) {
            /*int indice = 0;
            double newPourcentage = pourcentageApparition;
            ...
            pourcentageApparition = (int) ((Math.random() + (1 * (pourcentage - 75))) * 4);
            while (indice < this.pokemons.size()) {
                if (this.environnements.get(indice).equals(environnement)) {
                    if (pourcentageApparition <= (this.pourcentage.get(indice) + newPourcentage)) {
                        return this.pokemons.get(indice); 
                    }
                    newPourcentage += this.pourcentage.get(indice);
                }
                indice++;
            }
        }
