...
abstract public class Batiments extends Quartier implements IBatiment {
    ...
    public Batiments(String nom, int positionX, int positionY, int longueur, int largeur, int etageMin, int etageMax, Orientation orientation, Quartier endroitBatiment) {
    ...
    public void entrer(Dresseur equipe) {
        ...
        if (objet_endroit.getPosition().getPositionX() == 0 || this.orientation == Orientation.dos) {
            p.reculer();
        } else if (objet_endroit.getPosition().getPositionY() == 0 || this.orientation == Orientation.gauche) {
            p.droite();
        } else if (objet_endroit.getPosition().getPositionX() == salle.getLargeur() -1 || this.orientation == Orientation.face) {
            p.avancer();
        } else {
            p.gauche();
        }
    ...
    public void ajouterPokemon (Pokemon pk, Environnement environnement, int pourcentage) {
        for (ArrayList<Salle> list : this.salles) {
            for (Salle salle : list) {
                salle.ajouterPokemon(pk, environnement, pourcentage);
            }
        }
    }
}