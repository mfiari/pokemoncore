...
public class AttributsExp implements Serializable {
    ...
    private final Map<Attributs, Integer> attribut_niv;
    private final Map<Attributs, Integer> attribut_exp;
    private final int tableauExpParNiveau[];
    
    public AttributsExp () {
        this.attribut_niv = new EnumMap<>(Attributs.class);
        for (Attributs value : Attributs.values()) {
            this.attribut_niv.put(value, 1);
        }
        this.attribut_exp = new EnumMap<>(Attributs.class);
        for (Attributs value : Attributs.values()) {
            this.attribut_exp.put(value, 0);
        }
    ...
    public AttributsExp (AttributsExp attributsExp) {
        this.attribut_niv = new EnumMap<>(Attributs.class);
        for (Attributs value : Attributs.values()) {
            this.attribut_niv.put(value, attributsExp.attribut_niv.get(value));
        }
        this.attribut_exp = new EnumMap<>(Attributs.class);
        for (Attributs value : Attributs.values()) {
            this.attribut_exp.put(value, attributsExp.attribut_exp.get(value));
        }
        this.tableauExpParNiveau = new int[5];
        this.tableauExpParNiveau[0] = 100;
        this.tableauExpParNiveau[1] = 300;
    ...
    public void setAllNiveau (int niv) {
        for (Attributs value : Attributs.values()) {
            this.attribut_niv.put(value, niv);
        }
    }
